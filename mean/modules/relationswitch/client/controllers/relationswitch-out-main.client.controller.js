(function () {
  ...
  angular
    ...
    .controller('RelationswitchOutMainController', RelationswitchOutMainController);

  RelationswitchOutMainController.$inject = ['$scope', 'Notification', '$log', '$window',
    '$stateParams', 'RelationswitchOutService', '$uibModal', 'Upload'];
  function RelationswitchOutMainController($scope, Notification, $log, $window,
                                           $stateParams, RelationswitchOutService, $uibModal, Upload) {
    var vmo = this;
    //获取参数
    ...
    vmo._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/relationswitch/client/views/relationswitch-out-modal-form.client.view.html',
        controller: 'RelationswitchOutModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vmo.add = function () {
      var modalInstance = vmo._openModal({
        RelationswitchOutData: function () {
          //空数据
          return new RelationswitchOutService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/relationswitchout',
          data: result
        })
        // result.$save()
          .then(function (res) {
            refreshRecordCount(vmo.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党员关系转出新增成功!'});
          })
          .catch(function (err) {
            $log.error('dj_MemberRelationOut add save error:', err.data.message);
            Notification.error({
    ...
    vmo.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vmo.selectedRow.$remove(function () {
          ...
          //复位当前行
          vmo.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党员关系转出删除成功!'});
        })
          .catch(function (err) {
            $log.error('dj_MemberRelationOut deleted error:', err.data.message);
            Notification.error({
              message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
              ' 党员关系转出删除失败!'
            });
    ...
    vmo._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/relationswitchout/' + result.shipId,
            data: result
          })
          // result.$update()
            .then(function (res) {
              ...
              refreshRecordCount(vmo.queryParam);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党员关系转出修改成功!'});
            })
            .catch(function (err) {
              $log.error('dj_MemberRelationOut update save error:', err.data.message);
              Notification.error({
                message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
                '党员关系转出修改失败!'
              });
    ...
    function refreshRecordCount(queryParam) {
      RelationswitchOutService.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return RelationswitchOutService.query(pageParam).$promise
        .then(function (data) {
